...
import { X, ExternalLink, RefreshCw, MapPin, Camera, Maximize2, Minimize2 } from 'lucide-react';
...
/**
 * IframeScaler
 * Renders an iframe inside a container, then CSS-transform-scales it down
 * so the full page content fits within the visible panel — no top-left crop.
 *
 * Strategy:
 *  1. Render the iframe at NATIVE_W × NATIVE_H (a reasonable default viewport
 *     that most RTSP-proxy pages are designed for).
 *  2. Use CSS `transform: scale(ratio)` + `transform-origin: top left` to
 *     shrink it down to fit the actual container dimensions.
 *  3. The outer wrapper is sized to the container; the inner iframe is
 *     positioned absolute at native size then scaled — this avoids scrollbars
 *     and the overflow-crop problem.
 */
const NATIVE_W = 1280; // px — assumed native width of the proxy stream page
const NATIVE_H = 720;  // px — assumed native height (16:9)

function IframeScaler({ src }: { src: string }) {
  ...
  const recalc = useCallback(() => {
    ...
    // Fit-contain: pick the smaller scale so the whole page is visible
  ...
  return (
    // Outer wrapper fills the container and clips overflow
    <div
      ref={wrapperRef}
      className="absolute inset-0 overflow-hidden bg-black"
    >
      {/* Inner iframe rendered at native size, scaled to fit */}
      <iframe
        src={src}
        style={{
          width: NATIVE_W,
          height: NATIVE_H,
          border: 'none',
          // Scale from top-left so the whole frame is visible, centred
          transform: `scale(${scale}) translate(${((wrapperRef.current?.offsetWidth ?? 0) / scale - NATIVE_W) / 2}px, ${((wrapperRef.current?.offsetHeight ?? 0) / scale - NATIVE_H) / 2}px)`,
          transformOrigin: 'top left',
          // Prevent the iframe itself from showing scroll bars at native size
          overflow: 'hidden',
          pointerEvents: 'auto',
        }}
...
export default function CameraViewer({ camera, onClose, onLocate }: CameraViewerProps) {
  ...
  useEffect(() => {
    ...
    // iframe: no loading spinner needed — iframe handles its own load state
    if (streamType === 'iframe' && camera.stream_url) {
      setLoading(false);
      return;
    }

    // JPG snapshot fallback
    if (camera.feed_url) {
  ...
  // Auto-refresh for JPG snapshots
  useEffect(() => {
        ...
        >
          ...
          <div className="flex items-center justify-between px-3 md:px-4 py-2 md:py-3 border-b border-[var(--border-secondary)] bg-black/40 flex-shrink-0">
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              ...
              >
                {fullscreen
                  ? <Minimize2 className="w-3 h-3 text-[var(--text-muted)] hover:text-white" />
          ...
          {/*
            ...
            - Minimised panel: fixed aspect-video (16:9) on desktop, capped at 35vh on mobile
            - Fullscreen: flex-1 fills remaining height after header + footer
            The `relative` wrapper lets IframeScaler / video / img fill 100% cleanly.
          */}
          <div
            className={`relative bg-black flex-shrink-0 ${
              fullscreen
                ? 'flex-1'           // fills all remaining height
                : 'aspect-video max-h-[35vh] md:max-h-none md:aspect-video'
            }`}
          >
            {/* Loading spinner — only for non-iframe types */}
            {loading && !error && !externalOnly && streamType !== 'iframe' && (
            ...
            {externalOnly ? (
              ...
              /*
                IframeScaler renders the proxy page at its native 1280×720 resolution
                then CSS-scales it down to fit the panel.
                This eliminates the top-left crop that happens when a plain
                `width:100% height:100%` iframe overflows its container.
              */
              <IframeScaler src={camera.stream_url} />
            ...
            {/* Live badge — shown for all active (non-error, non-external) streams */}
            {!error && !externalOnly && (
              <div className="absolute top-2 left-2 flex items-center gap-1.5 bg-black/70 backdrop-blur-sm px-2 py-1 rounded pointer-events-none z-10">
                ...
                <span className="text-[7px] font-mono text-white tracking-widest">
                  {streamType === 'jpg' ? 'LIVE SNAPSHOT' : streamType === 'iframe' ? 'LIVE STREAM' : 'LIVE VIDEO'}
                </span>
              </div>
            )}
          </div>
