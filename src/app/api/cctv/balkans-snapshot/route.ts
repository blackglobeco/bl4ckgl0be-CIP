...
/**
 * Generic Image Proxy — Balkans / Eastern Europe
 *
 ...
 *   - webcam.asfinag.at        (Austria — ASFINAG secondary image CDN)
 *   - www.asfinag.at           (Austria — ASFINAG primary image host)
 *
 * NOTE: images-webcams.windy.com is NOT included. Windy's snapshot CDN
 * requires a signed API key token (x-windy-api-key) and returns HTTP 403
 * unconditionally without one. No proxy can fix this — use the Windy embed
 * iframe (stream_url) instead and omit feed_url for Windy cameras.
 *
 ...
 *   - Content-Type validated to be image/* before forwarding
 */
